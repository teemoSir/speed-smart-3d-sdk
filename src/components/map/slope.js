...
class SlopeEffect {
    ...
    showResult(startPoint, endPoint) {
        ...
        for (var i = 1; i <= pointSum; i++) {
            ...
            //console.log((disL[i - 1] + dis).toFixed(2),typeof(disL[i - 1] + dis));
            let num = disL[i - 1] + dis;
            disL.push(Number(num.toFixed(2)));
            angle = Math.asin((tempCartographic.height - lastCarto.height) / dis);
            slopePercent.push(Number((Math.tan(angle) * 100).toFixed(2)));
        }

        _self.echartContainer = document.createElement('div');
        _self.echartContainer.className = 'echart-viewer';
        _self.viewer.container.appendChild(_self.echartContainer, 'dark', {
            renderer: 'canvas',
            width: 640,
            height: 480
        });
        _self.echartContainer.style.position = "absolute";
        _self.echartContainer.style.right = '160px';
        _self.echartContainer.style.backgroundColor = 'rgba(23, 49, 71, 0.5)';
        _self.echartContainer.style.boxShadow = '0 0 0.6rem #ffffff inset';
        _self.echartContainer.style.top = '80px';
        _self.echartContainer.style.height = '300px';
        _self.echartContainer.style.width = '640px';
        _self.echartContainer.style.overflow = "hidden";
        _self.echartContainer.style.zIndex = "9999";
        _self.echartContainer.style.opacity = 0.9;
        var myChart = echarts.init(_self.echartContainer);
        var option = {
            title: {
                text: '剖面示意图',
                left: 'center',
            ...
            series: [
                {
                    ...
                    markPoint: {
                        data: [
                            {type: 'max', name: '最大值',formatter:function(data){return data.toFixed(2)}},
                            {type: 'min', name: '最小值'}
                        ]
                    },
                    markLine: {
