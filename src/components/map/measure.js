...
export default class Measure {
    ...
    constructor(core) {
        ...
        this.areaSpace = {
            ...
            getArea(points) {
                ...
                for (var i = 0; i < points.length - 2; i++) {
                    ...
                    var dis_temp1 = this.calcDistance(this.positions[i], this.positions[j]);
                    var dis_temp2 = this.calcDistance(this.positions[j], this.positions[k]);
        ...
        this.triangleSpace = {
            ...
            createLine(_self) {
                _self.triangleSpace.lineObj[0] = new entityFactory({
                ...
                _self.triangleSpace.lineObj[1] = new entityFactory({
                    type: "triangleMeasure",
                    data: {
                        name: '高度',
                ...
                _self.triangleSpace.lineObj[2] = new entityFactory({
                    type: "triangleMeasure",
                    data: {
                        name: '水平线',
    ...
    remove() {
        ...
        if (this.triangleSpace.entity.length > 0) {
            ...
            this.triangleSpace.distance= 0;
            this.triangleSpace.lineObj= [];
            this.triangleSpace.entity= [];
            this.triangleSpace. position1= [];
            this.triangleSpace.position2= [];
            this.triangleSpace.position3= [];

        }
        if (this.lineSpace.entity.length > 0) {
            for (let i in this.lineSpace.entity)
                this.entitys.remove(this.lineSpace.entity[i]);
            this.lineSpace.distance= 0;
            this.lineSpace.lineObj= undefined;
            this.lineSpace.positions= [];
            this.lineSpace.entity= [];
        }
                ...
                if (_self.areaSpace.positions.length >= 2) {
                    if (!Cesium.defined(_self.areaSpace.polyObj)) {
                        _self.areaSpace.polyObj = new entityFactory({
                            type: "createPolygon",
                            data: {
                                positions: _self.areaSpace.positions,
        ...
        _self.handlerAction.Action(function (e) {
            ...
            if (_self.lineSpace.positions.length >= 2) {
                if (!Cesium.defined(_self.lineSpace.lineObj)) {
                    _self.lineSpace.lineObj = new entityFactory({
                        type: "createLine",
                        data: {positions: _self.lineSpace.positions, clampToGround: true}
                    });
